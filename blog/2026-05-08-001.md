---
title: "工程实践：基于大模型的验证码识别与登录爆破自动化"
description: "本文通过 Vulinbox 授权测试场景，演示如何用 Yaklang ai.FunctionCall 识别验证码，并将基础版、并发版脚本与 Web Fuzzer 热加载串成可验证的自动化流程。"
date: 2026-05-08
authors: [yak]
tags: ["2026", "ai"]
slug: llm-captcha-recognition-and-login-brute-force
---
## 内容速览

- **适用场景：** 已获授权的内部渗透测试、验证码逻辑验证和安全工具研发。
- **实现方法：** 使用 `ai.FunctionCall` 与 `ai.imageBase64` 提取验证码，再分别接入基础脚本、并发任务和 Web Fuzzer 热加载流程。
...
**3.AI 识别验证码 / 4.改写传入的请求**：把请求中的 `YSESSIONID_SAFE` Cookie 替换为新的，把 POST 参数 `code` 替换为识别结果
