...
slug: sql-injection-range-part2
---
**前言**

<!-- truncate -->

上一篇我们讲解了部分 SQL 注入漏洞类型[Yakit靶场通关教程|SQL注入篇（一）](http://mp.weixin.qq.com/s?__biz=Mzk0MTM4NzIxMQ==&mid=2247512880&idx=1&sn=3e865cd59b7820e4d1a701f2cb78ac2a&chksm=c2d1cf94f5a64682433d2278a9320a7ac712d8f3ff92ff9f41da522a8b16244b0a47ae4f7987&scene=21#wechat_redirect) 。一个严重的 SQL 注入漏洞，可能给企业带来重大损失。

这并不是玩笑话。SQL注入漏洞不仅普遍并且危害也极高，类型繁杂，作为网安从业者，应该能认识到**SQL注入对于安全测试的重要性**。

那么本周我们将开启进阶之旅，了解更高难度的SQL注入类型。在SQL注入中，可以简单分为**数字型、字符型**两种类型。如何判定呢？看\斜杠后面跟着的字符，是什么字符，它的闭合字符就是什么，若是没有，就为数字型。上一篇我们谈到了数字型，本文将着重讲解**字符型注入**。
...
SQL注入虽说是一种流行的攻击方法，但是通过采取适当的预防措施也是可以进行防范的。本文在每一种类型后都附上了具体的**防御措施**，可在靶场练习过程中同步记录。
