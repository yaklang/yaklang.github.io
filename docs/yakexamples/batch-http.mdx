...
1. [WEB 模糊测试入门：概念与基础使用](/docs/newforyak/fuzz_tutorial)
1. [WEB 模糊测试进阶：参数细节处理](/docs/newforyak/fuzz_for_more)
1. [WEB 模糊测试手册：从 fuzz 标签到链式请求](/docs/buildinlibs/lib_fuzz)
...
我们在使用 `*mutate.FuzzHTTPRequest` 的的时候，如果不知道他的定义是啥，有哪些可用方法，可以通过 `desc(freq)` 直接查看
