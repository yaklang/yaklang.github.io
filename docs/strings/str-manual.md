...
需要"命中任意一个 / 全部"多个模式时，`str` 提供了成组的匹配函数：`MatchAnyOf*` 任意命中即真，`MatchAllOf*` 全部命中才真。子串形态（`...SubString`）**不区分大小写**，另有通配符（`...Glob`，`*` 任意长度、`?` 单字符）与正则（`...Regexp`）两种形态；`str.RegexpMatch(pattern, s)` 则是单条正则匹配。

``````````````yak
// 特性: 多模式匹配 str.MatchAnyOf* / MatchAllOf* / RegexpMatch
// 关键词: str.MatchAnyOfSubString, str.MatchAnyOfGlob, str.MatchAllOfRegexp
// 子串匹配不区分大小写: "abcDEF" 含 "a"(任意命中即真)
println(str.MatchAnyOfSubString("abcDEF", "a", "z", "x"))   // OUT: true
// 全部子串都要命中
println(str.MatchAllOfSubString("abc", "a", "b", "c"))      // OUT: true

// 通配符 glob: * 匹配任意长度, ? 匹配单字符
println(str.MatchAnyOfGlob("abc", "a*", "??b"))             // OUT: true
println(str.MatchAllOfGlob("abc", "a*", "?b?"))             // OUT: true

// 正则: 任意命中 / 全部命中
println(str.MatchAnyOfRegexp("abc", "a.+", "Ab.?"))         // OUT: true
println(str.MatchAllOfRegexp("abc", "a.+", ".?b.?"))        // OUT: true

// RegexpMatch 单条正则匹配(参数顺序是 pattern, s)
println(str.RegexpMatch("^[a-z]+$", "abc"))                 // OUT: true
``````````````
...
面向扫描目标，`str` 内置了一组主机、端口与 IP 处理函数：`ParseStringToHostPort` 从 `host:port` 或 URL 拆出主机与端口（URL 缺省端口按 scheme 推断，返回 `(host, port, err)`）；`IsIPv4` / `IsIPv6` 判断 IP 版本；`ExtractHost` / `ExtractRootDomain` 从 URL、域名中抽主机名与根域名；`SplitHostsToPrivateAndPublic` 把目标分成内网与公网两组；`IPv4ToCClassNetwork` 把 IP 归一到 C 段 CIDR（返回 `(string, err)`）。

``````````````yak
// 特性: 主机/端口/IP 解析 str.ParseStringToHostPort / IsIPv6 / ExtractRootDomain ...
// 关键词: str.ParseStringToHostPort, str.SplitHostsToPrivateAndPublic, str.IPv4ToCClassNetwork
// 从 URL 拆主机与端口, https 缺省端口推断为 443; 用 ~ 抹掉 err
host, port = str.ParseStringToHostPort("https://example.com")~
println(host)   // OUT: example.com
println(port)   // OUT: 443

// IP 版本判断
println(str.IsIPv6("2001:db8::1"))   // OUT: true
println(str.IsIPv4("192.168.1.1"))   // OUT: true

// 从 URL 抽主机名; 从多级域名抽根域名(返回列表)
println(str.ExtractHost("http://www.example.com/path"))   // OUT: www.example.com
println(str.ExtractRootDomain("a.b.example.com"))         // OUT: [example.com]

// 把目标分成内网/公网两组
priv, pub = str.SplitHostsToPrivateAndPublic("127.0.0.1,example.com")
println(priv)   // OUT: [127.0.0.1]
println(pub)    // OUT: [example.com]

// 归一到 C 段 CIDR(返回 (string, err), 用 ~ 处理)
println(str.IPv4ToCClassNetwork("192.168.1.55")~)   // OUT: 192.168.1.0/24
``````````````
