...
fuzz 是在二进制领域最常见的操作之一，零零星星也有人在 web 领域提到这个概念和操作，但是我们并没有找到特别棒的 web fuzz 手段。

长久以来我们会困在工具中迷失方向，webfuzz 相关的工具其实早就有了，大多针对一个 HTTP 请求进行变形操作；其实这个概念在 Burpsuite 的 Intruder 中很常见了。

例如，我们会使用 Intruder 针对某一个参数的字段使用预设字典进行批量请求访问；或者针对 path 部分进行批量请求，来检查结果分析，分析网站可能存在的访问路径。
