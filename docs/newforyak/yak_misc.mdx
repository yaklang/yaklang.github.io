...
1. [`subdomain`的使用教程](/docs/buildinlibs/lib_subdomain)
1. [`subdomain`所有可用API与参数配置手册](/docs/api/subdomain)
...
如果用户不熟悉 `subdomain` 中有啥字段可以使用，可以参考文档 [`subdomain` 中的结果处理](/docs/buildinlibs/lib_subdomain#核心函数与基础使用)
...
1. [`file` 文件读写](/docs/buildinlibs/lib_file)
