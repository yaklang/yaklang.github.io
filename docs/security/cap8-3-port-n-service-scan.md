...
本章的 `synscan`/`servicescan` 示例都是**真实网络扫描**：`synscan` 需要原始套接字权限（通常需 root/管理员），`servicescan` 会对外部主机发起连接，二者均无法在文档里离线确定性地自动验证，因此作为展示性代码给出，扫描结果以正文输出示例为准。Yak 代码统一使用 `yak` 高亮，函数签名与结构体定义使用 `text`。
:::
