---
sidebar_position: 2
sidebar_label: "HTTP 协议与数据包处理（poc）"
---
...
- **可离线验证的示例**（报文解析、改包、参数发现等）：使用 `assert` 锁定确定性结论，并用 `println(...) // OUT: 值` 标注真实标准输出，均已 `yak` 实跑验证。
- **网络类示例**（`poc.HTTP`、`Exec` 等真实发包）：依赖外部网络，保留为展示性代码，输出以 `/* ... */` 注释给出参考结果。
- **HTTP 报文示意**：使用 `http` 高亮，仅用于讲解报文结构，并非可执行的 Yak 代码。

所有 Yak 代码块统一使用 `yak` 语言高亮。
:::
...
``````````````yak
// 特性: 代理认证 URL 中的特殊字符需要先做 URL 编码
// 关键词: codec.EscapeQueryUrl, poc.proxy
// codec.EscapeQueryUrl 会把用户名中的特殊符号编码, 避免干扰代理 URL 解析
println(codec.EscapeQueryUrl("admin111//;@")) // OUT: admin111%2F%2F%3B%40

proxy = "http://%v:%v@127.0.0.1:8083" % [codec.EscapeQueryUrl("admin111//;@"), "123456"]
assert proxy == "http://admin111%2F%2F%3B%40:123456@127.0.0.1:8083", "proxy url should embed url-encoded username"
println(proxy) // OUT: http://admin111%2F%2F%3B%40:123456@127.0.0.1:8083

// 把构造好的 proxy 传给 poc.proxy 即可(此处不实际发包):
// rsp, req = poc.HTTP(packet, poc.https(true), poc.proxy(proxy))~
``````````````
...
``````````````yak
// 特性: poc 报文解析 (路径 / Cookie / Header / Query 参数)
// 关键词: poc.GetHTTPRequestPath, poc.GetHTTPPacketCookie, poc.GetHTTPPacketHeader, poc.GetHTTPPacketQueryParam
...
// 完整路径(含 query) 与 不含 query 的路径
println(poc.GetHTTPRequestPath(packet))             // OUT: /mng/index.html?key=value&key2=foo&key3=bar
println(poc.GetHTTPRequestPathWithoutQuery(packet)) // OUT: /mng/index.html

// 按名取 Cookie
println(poc.GetHTTPPacketCookie(packet, "cookieId")) // OUT: 123
println(poc.GetHTTPPacketCookie(packet, "b"))        // OUT: 1

// 取 Header (大小写不敏感)
println(poc.GetHTTPPacketHeader(packet, "user-agent")) // OUT: test-agent

// 取单个 GET 查询参数
println(poc.GetHTTPPacketQueryParam(packet, "key"))  // OUT: value
println(poc.GetHTTPPacketQueryParam(packet, "key2")) // OUT: foo

// 取全部查询参数(返回 map)
params = poc.GetAllHTTPPacketQueryParams(packet)
assert params["key"] == "value", "query param key should be value"
assert params["key3"] == "bar", "query param key3 should be bar"
println(params) // OUT: map[key:value key2:foo key3:bar]
``````````````
...
``````````````yak
// 特性: 从 HTTP 请求报文提取完整 URL
// 关键词: poc.GetUrlFromHTTPRequest
packet = `GET /mng/index.html?key=value&key2=foo&key3=bar HTTP/1.1
Host: www.example.com
User-Agent: test-agent
Cookie: cookieId=123; b=1
...
assert url == "http://www.example.com/mng/index.html?key=value&key2=foo&key3=bar", "url should be assembled from first-line and Host"
println(url) // OUT: http://www.example.com/mng/index.html?key=value&key2=foo&key3=bar
``````````````
...
``````````````yak
// 特性: 修复不规范报文(把 \n 补成标准 CRLF, 并补全结尾空行/Content-Length)
// 关键词: poc.FixHTTPRequest
packet = "GET / HTTP/1.1\nHost: www.example.com"
fixed = poc.FixHTTPRequest(packet)
// 修复后使用标准 CRLF
assert string(fixed).Contains("GET / HTTP/1.1\r\n"), "fixed packet should use CRLF"
assert string(fixed).Contains("Host: www.example.com\r\n"), "Host line should end with CRLF"
dump(fixed)
``````````````
...
/*
GET /prefixA HTTP/1.1
Host: www.example.com
Content-Length: 5

abc=1
GET /prefixB HTTP/1.1
Host: www.example.com
Content-Length: 5

abc=1
GET /prefixC HTTP/1.1
Host: www.example.com
Content-Length: 5
...
/*
// 注意: boundary 为随机生成, 每次运行结果不同, 这里仅作结构示意
GET / HTTP/1.1
Host: www.example.com
...
``````````````yak
// 特性: 自动发现 GET 参数
// 关键词: fuzz.HTTPRequest, GetGetQueryParams
...
// 收集参数名(顺序不保证, 排序后断言)
names = []
for param in params { names = append(names, param.Name()) }
names.Sort()
assert len(params) == 2, "should discover 2 GET params"
println(str.Join(names, ",")) // OUT: get1,get2
``````````````
...
``````````````yak
// 特性: 自动发现 Cookie 参数
// 关键词: fuzz.HTTPRequest, GetCookieParams
raw = `POST /?get1=1&get2=2 HTTP/1.1
Host: www.example.com
Content-Type: application/x-www-form-urlencoded
Cookie: cookie1 = 1;cookie2 = 2
...
for param in params { names = append(names, param.Name()) }
names.Sort()
assert len(params) == 2, "should discover 2 cookie params"
println(str.Join(names, ",")) // OUT: cookie1,cookie2
``````````````
...
``````````````yak
// 特性: 自动发现键值对(application/x-www-form-urlencoded) POST 参数
// 关键词: fuzz.HTTPRequest, GetPostParams
raw = `POST /?get1=1&get2=2 HTTP/1.1
Host: www.example.com
Content-Type: application/x-www-form-urlencoded
Cookie: cookie1 = 1;cookie2 = 2
...
for param in params { names = append(names, param.Name()) }
names.Sort()
assert len(params) == 2, "should discover 2 post params"
println(str.Join(names, ",")) // OUT: post1,post2
``````````````
...
``````````````yak
// 特性: 自动发现 application/json 请求体参数
// 关键词: fuzz.HTTPRequest, GetPostJsonParams
raw = `POST / HTTP/1.1
Host: www.example.com
Content-Type: application/json
...
for param in params { names = append(names, param.Name()) }
names.Sort()
assert len(params) == 2, "should discover 2 json body params"
println(str.Join(names, ",")) // OUT: a,c
``````````````
...
``````````````yak
// 特性: 一次性发现常用参数(GET / Cookie / 请求体, 优先解析 JSON)
// 关键词: fuzz.HTTPRequest, GetCommonParams
raw = `POST /?get1=1&get2=2 HTTP/1.1
Host: www.example.com
Content-Type: application/x-www-form-urlencoded
Cookie: cookie1 = 1;cookie2 = 2
...
for param in params { names = append(names, param.Name()) }
names.Sort()
assert len(params) == 6, "should discover 6 common params"
println(str.Join(names, ",")) // OUT: cookie1,cookie2,get1,get2,post1,post2
``````````````
