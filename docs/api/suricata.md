...
| [suricata.ParseSuricata](#parsesuricata) |ParseSuricata 解析 Suricata 规则文本，返回结构化的规则对象列表 在 yak 中通过 suricata.ParseSuricata 调用 参数: - raw: Suricata 规则文本，可包含一条或多条规则 返回值: - 解析得到的规则对象列表 - 错误信息，解析失败时非 n...|
...
`ParseSuricata(raw string) ([]*surirule.Rule, error)`
