...
1. 判断字符串以指定字符开始 `fn str.StartsWith(var_1: string, var_2: string): bool`
1. 把字符串全部小写，并去除左右空白 `fn str.LowerAndTrimSpace(var_1: string): string`
1. Split之后把元素两边的空白移除 `fn str.SplitAndTrim(var_1: string, var_2: string): []string`
1. 把 Json 字符串转换成 `map[string]string` `fn str.JsonToMap(var_1: string): map[string]string`
1. 把 Json 字符串转换成 `[]map[string]string` `fn str.JsonToMapList(var_1: string): []map[string]string`
1. 使用 io.Reader 读取 Json 数据流 `fn str.JsonStreamToMapList(var_1: io.Reader): []map[string]interface `&#123;&#125;
1. 把字符串数组作为 Path 进行拼接`fn str.PathJoin(vars: ...string): string`
1. 正则匹配 `fn str.RegexpMatch(var_1: interface {}, var_2: string): bool`
1. 把一个对象转变成 Json 字符串，做好锁进展示 `fn str.ToJsonIndentStr(var_1: interface {}): string`
...
1. 判断 StringSlice 中是不是包含指定元素 `fn str.StringSliceContains(var_1: []string, var_2: string): bool`
1. 判断 StringSlice 中是不是包含后续所有字符串`fn str.StringSliceContainsAll(var_1: []string, vars: ...string): bool`
1. 移除 `List/Slice` 中重复的元素`fn str.RemoveRepeat(var_1: []string): []string`
1. 从一个字典中取 key 对应的值，如果没有，设置为 `idEmptyDefault` 的默认值`fn str.ParamsGetOr(var_1: map[string]string, key: string, ifEmptyDefault: string): string`
...
1. 随机生成一个指定长度的随机字符串，可作为密码 `fn str.RandSecret(var_1: int): string`
1. 随机生成一个指定长度字符串 `fn str.RandStr(var_1: int): string`


### 特殊操作

1. 创建一个字符串去重工具 `fn str.NewFilter(): *filter.StringFilter`
1. **【重要】**：Grok 支持 `fn str.Grok(var_1: string, var_2: string): yaklib.GrokResult`
