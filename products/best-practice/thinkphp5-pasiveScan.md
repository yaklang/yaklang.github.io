# thinkphp5 rce被动扫描插件[mitm]



## 环境搭建

环境使用http://vulfocus.io/
...
所以当`http://xxx.xx/index.php?c=4e5e5d7364f443e28fbf0d3ae744a59a`返回一个图片时，就可以判定这是thinkphp站点
...
可以通过[fuzz标签](https://www.yaklang.io/docs/buildinlibs/lib_fuzz#fuzz-%E6%A0%87%E7%AD%BE%E5%AE%9A%E4%B9%89%E4%BB%A5%E5%8F%8A%E4%BD%BF%E7%94%A8)去写payload，如
...
testExp = fn(rootUrl){
    ...
    for _,payload = range payloads{
        ...
        if str.MatchAllOfSubString(body, result){
            yakit_output(sprintf("%s存在漏洞, pocName:%s, OS: Linux",rootUrl,payload["name"]))
        }
        ...
        if str.MatchAllOfSubString(body, "system.ini"){
            yakit_output(sprintf("%s存在漏洞, pocName:%s, OS: Windows",rootUrl,payload["name"]))
        }
    }
}
...
mirrorNewWebsite = func(isHttps /*bool*/, url /*string*/, req /*[]byte*/, rsp /*[]byte*/, body /*[]byte*/) {
    rootUrl = str.ParseStringUrlToWebsiteRootPath(url)
    headers, body = str.SplitHTTPHeadersAndBodyFromPacket(rsp)
    if checkHeader(headers) || chechIcon1(rootUrl){
        testExp(rootUrl)
    }
