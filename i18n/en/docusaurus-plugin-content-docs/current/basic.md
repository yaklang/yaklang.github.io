...
# Basic Syntax: Aim for 10 minutes to get started

Yak is a pure Golang embedded language, a pure Golang implementation of a language based on reflection features, with a syntax that retains a certain degree of Golang style and even enables the seamless opening of Golang objects for use by Yak.

Some of the amazing features of Golang we can also find implementation in Yak, for example, you may be familiar with or heard of `go func() {} ()`, the `defer` keyword, `chan <T>`, etc.

While retaining these great features, we have greatly simplified Golang's syntax by removing the statically typed features, removing strict syntax checking, and making it a dynamically strongly typed language, while trying to be compatible with some of the common usage scenarios of dynamic languages.

**The most exciting thing** is actually that we have all kinds of security scanning tools and unprecedented fuzz testing modules built into the language that security practitioners need.

We hope that Yak will be quite friendly to security professionals and you can get started in 10 minutes! If you're familiar with Golang and Python, I'm sure Yak will be a glue language you'll love as well.

> For yak there is no need to add a semicolon at the end of each line, but of course it doesn't matter if you do
