...
| [ai.IntelligentChat](#intelligentchat) | IntelligentChat uses the intelligent (high-quality) model |
| [ai.IntelligentFunctionCall](#intelligentfunctioncall) |  |
| [ai.LightweightChat](#lightweightchat) | LightweightChat uses the lightweight (fast) model |
| [ai.LightweightFunctionCall](#lightweightfunctioncall) |  |
...
| [ai.VisionChat](#visionchat) | VisionChat uses the vision model |
| [ai.VisionFunctionCall](#visionfunctioncall) |  |
...
| [ai.visionPriority](#visionpriority) |  |
...
- 描述: IntelligentChat uses the intelligent (high-quality) model
...
- 描述: LightweightChat uses the lightweight (fast) model
...
- 描述: VisionChat uses the vision model
...
|参数名|参数类型|参数解释|
|:-----------|:---------- |:-----------|
| cb | `RawHTTPResponseHeaderCallback` |   |
...
- 详细描述: 不再被gateway拍平为单条user消息。行为：当RawMessages非空时，gateway的Chat(s)会把它带入ChatBaseContext.RawMessages，chatBaseChatCompletions跳过单user包装，直接以RawMessages作为最终请求体的messages字段。主要使用场景:aibalance等中转层希望保留客户端messages的role顺序与content结构，最大化上游隐式缓存的前缀命中率。关键词:WithRawMessages,messages完整透传,隐式缓存前缀稳定
