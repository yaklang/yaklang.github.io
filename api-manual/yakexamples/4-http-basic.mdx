...
1. [函数简化调用：WavyCall](/docs/yak-basic/cap6-2-error-handling#664-使用try-catch处理崩溃)
...
大家可以在 [字符串：模版语法 f-string](/docs/yak-basic/cap3-stats-vars-and-exprs/#字符串模版字面量f-string) 这里复习这个特性。熟练应用这个特性，它会让你的编程更加流畅。
...
如果想要发送任意的畸形数据包，我们依赖 `http` 这个库无法做到，用户需要学习 [`poc` 与 `fuzz` 模糊测试函数库支持](/api-manual/yakexamples/http-advanced)。