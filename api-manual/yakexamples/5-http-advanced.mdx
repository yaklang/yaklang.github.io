...
在这些需求中，我们的视角将发生变化，在 [基础 HTTP 通信](/api-manual/yakexamples/http-basic) 中，我们需要以遵循 HTTP 协议的角度思考问题；但是在本文中，我们视角将变为对 "HTTP 数据包"。
...
1. [yak 中的并发编程](/api-manual/yakexamples/concurrent)

### 方法二：学习使用 `fuzz` - 模糊测试工具包

当然，您可以继续学习更强大的[模糊测试技术 `fuzz` 模块](/api-manual/yakexamples/fuzz-request)来掌握如何测试多个数据包。
